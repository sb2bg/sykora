...
pub const Uci = struct {
    ...
    fn handleCommand(self: *Self, command: ToEngineCommand) UciError!void {
        switch (command) {
            ...
            .go => |go_opts| {
                // Capture the clock immediately: thread spawn, worker setup,
                // and table initialization all happen on the GUI's time.
                const go_received = std.time.Instant.now() catch unreachable;
                try smp.terminateSearch(self);
