#!/usr/bin/env python3
"""Run a small Bullet CPU-backend training test on Apple Silicon (or any CPU).

This is intended to validate dataset/tooling and training plumbing,
not to produce final-strength nets.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="End superbatch (keep tiny for local CPU sanity)",
    )
    ...
    parser.add_argument(
        ...
        help="CPU training/data threads",
    )
    parser.add_argument(
        "--resume",
        default="",
...
def main() -> int:
    ...
    run_id = args.run_id.strip() or datetime.datetime.now(datetime.UTC).strftime("cpu_test_%Y%m%dT%H%M%SZ")
    ...
    print("CPU sanity training finished.")
