#!/usr/bin/env python3
"""Convert a float-domain NPZ checkpoint into Sykora SYKNNUE6 format.

Supports the SYKNNUE6 output-bucket ladder:
  * --output-bucket-count 1  -> scheme=single   (Stage 1 parity nets)
  * --output-bucket-count 8  -> scheme=material  (Stage 2+ nets)

Defaults to the proven v3 10-bucket input layout unless the NPZ carries an
explicit `bucket_layout_64`.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export NPZ checkpoint to SYKNNUE6 net."
    )
...
def main() -> int:
    ...
    with np.load(in_path) as ckpt:
        ...
        if "bucket_layout_64" in ckpt:
            ...
            bucket_layout = expand_mirrored_bucket_layout(V3_BUCKET_LAYOUT_32)
    ...
    if feature_set != FEATURE_SET_KING_BUCKETS_MIRRORED:
        raise ValueError("SYKNNUE6 only supports king_buckets_mirrored inputs")
    ...
    elif 32 % output_bucket_count == 0:
        output_bucket_scheme = OUTPUT_BUCKET_SCHEME_MATERIAL
    else:
        raise ValueError(
            f"output_bucket_count must be 1 or divide 32, got {output_bucket_count}"
        )
    ...
    scheme_name = "single" if output_bucket_scheme == OUTPUT_BUCKET_SCHEME_SINGLE else "material"
    print(f"Input: {in_path}")
    print("Output format: SYKNNUE6")
    print(f"Input bucket count: {max(bucket_layout) + 1}")
    print(f"Output bucket count: {output_bucket_count} (scheme={scheme_name})")
