...
usage() {
  cat <<'EOF'
Usage:
  utils/match/selfplay_vs_ref.sh <git-ref> [selfplay.py args...]

Description:
  Builds engine at <git-ref> in a temporary worktree, builds current working tree,
  then runs utils/match/selfplay.py as:
...
Examples:
  ...
  utils/match/selfplay_vs_ref.sh v0.2.0 --games 200 --depth 8 --openings default
EOF
}
